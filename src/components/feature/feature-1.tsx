...
const DEFAULT_CARDS: CardDataItem[] = [
  {
    id: 1,
...
export function Feature1({
  badge = "Features",
  title = "Blazingly fast to help your business grow",
  cards = DEFAULT_CARDS,
  collapseDelay = 5000,
  ltr = false,
  linePosition = "left",
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex((prevIndex) =>
        prevIndex !== undefined ? (prevIndex + 1) % cards.length : 0
      )
    }, collapseDelay)

    return () => clearInterval(timer)
  }, [currentIndex, cards.length, collapseDelay])

  useEffect(() => {
    const handleAutoScroll = () => {
      const nextIndex =
        (currentIndex !== undefined ? currentIndex + 1 : 0) % cards.length
  ...
  return (
    <section ref={ref} id="features">
      <div className="py-20 md:py-28">
        <div className="mx-auto flex w-full max-w-7xl flex-col items-center justify-center px-4 lg:px-8">
